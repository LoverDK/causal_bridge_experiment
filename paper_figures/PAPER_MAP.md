...
| Figure 4, NSW within-trial reconstruction | `legacy_audits/results/nsw_*`; paper asset `paper_figures/assets/figure5_nsw.pdf` | `legacy_audits/src/causal_atlas_sim/nsw_experiment.py`; `reproduce.py legacy-nsw` |
...
| Mechanism-set calibration audit | `current_method/results/mechanism_calibration/` | `current_method/run_mechanism_calibration.py`, `current_method/atlas_new/mechanism_calibration.py`; `reproduce.py calibration` |
| Other supplementary certificate, PI and bridge diagnostics | `current_method/results/full/`, `full_plus/`; retained extension records under `legacy_audits/results/extensions/` | `current_method/atlas_new/core.py`, `audit_plus.py`, and the retained extension modules |
...
The `paper_figures/assets/` files are the paper's figure inputs. The portable renderer regenerates Figures 5 and 6 from committed numerical records; PDF metadata and platform fonts can change file bytes. Figure 4's numerical records are regenerated by the NSW runner and its committed PDF is used as the paper input.

No superseded early prototype or unrelated dataset is required by the reproduction commands above.
