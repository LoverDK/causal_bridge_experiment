...
def main():
    ...
    parser.add_argument('suite',choices=['verify','current','plus','workflow','ablations','audit-ablations','real','calibration','original','legacy-nsw','legacy-extensions','figures'])
    ...
    if args.suite in ['current','plus','workflow','ablations','real','calibration']:
        dest=out/'current_method'
        shutil.copytree(ROOT/'current_method',dest,ignore=shutil.ignore_patterns('results','__pycache__'))
        script={'current':'run_all.py','plus':'run_plus.py','workflow':'run_workflow.py','ablations':'run_ablations.py','real':'run_real_supplement.py','calibration':'run_mechanism_calibration.py'}[args.suite]
        cmd=[script]
        if args.suite not in ['real','calibration']:
