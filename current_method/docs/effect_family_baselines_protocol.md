...
4. `study_split_conformal`：在训练池内按唯一 study analysis id（`study.analysis`）留一，校准绝对残差，再加目标 N-only allowance。`study.name` 是 family 标签，不能作为 study 单元。
...
逐目标 family 翻转 held-out `ESCI.d` 后重新运行，预测和 radius 必须不变。旧版结果曾误把 `study.name` 当作 study 单元，已保留作历史审计；修正版结果另存于 `results/effect_family_baselines_corrected/`。该面板提供了原评价要求的 baseline 对照和真实跨 family 压力测试，但不提供真实 `U_i`、独立现实 calibration archive 或无噪声 causal truth。十个 target 也不足以作稳定方法排名。下一步仍是预注册/新采集跨干预档案，并在 target outcome 揭盲前冻结机制代理和独立校准分割。
